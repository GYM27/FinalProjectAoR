...
public class EntityMapper {
    ...
    public AlertDTO toDto(Alert entity) {
        ...
        if (entity.getRule() != null) {
            ...
            dto.setFormattedValue(com.grupo3aor.innovationlab.util.ClinicalValueFormatter.formatClinicalMessage(entity));
    ...
    public EvaluationReportDTO toDto(EvaluationReport entity) {
        ...
        // We also extract the isLive status if we can infer it or if we need to save it.
        // In this case, we try to deduce if start/end are null for Batch vs Live, 
        // or if the simulation has a payload. But since EvaluationReport does not save isLive in the DB, 
        // To simplify, we let the frontend determine it.
        return dto;
    }
}
