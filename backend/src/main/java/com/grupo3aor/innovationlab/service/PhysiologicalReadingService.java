...
public class PhysiologicalReadingService {
    ...
    public static final int SMALL_BATCH_THRESHOLD = 1000;
    ...
    /**
     * Processes and persists a single physiological reading into the database.
     * <p>This method ensures the simulation is in a valid state (not finalized or canceled) 
     * before persisting the data. It then immediately broadcasts the new telemetry to all 
     * connected WebSocket clients subscribed to this simulation's topic, and triggers the 
     * {@link RuleEvaluatorService} to check for medical anomalies.</p>
     *
     * @param dto the physiological reading payload
     * @param userEmail the email of the user/system generating the reading
     * @param ipAddress the IP address of the source
     * @return the saved physiological reading as a DTO
     * @throws ResourceNotFoundException if the target simulation does not exist
     * @throws IllegalStateException if the target simulation is finalized or canceled
     */
    @Transactional
    ...
    /**
     * Asynchronously processes a telemetry reading.
     * <p>Used to offload high-frequency data ingestion from the main HTTP request threads 
     * into a dedicated {@code telemetryExecutor} thread pool, preventing thread starvation.</p>
     *
     * @param dto the physiological reading payload
     * @param userEmail the email of the user/system generating the reading
     * @param ipAddress the IP address of the source
    ...
    public List<PhysiologicalReadingDTO> createReadingBatch(List<PhysiologicalReadingDTO> dtos, String userEmail, String ipAddress) {
        ...
        if (entitiesToSave.size() < SMALL_BATCH_THRESHOLD) {
            List<PhysiologicalReading> savedReadings = new ArrayList<>();
            
            if (globalSettingsService.isDbFailed()) {
                // Degraded Mode: Save to RAM to avoid crashing the HTTP POST (Error 500) from the IoT script
