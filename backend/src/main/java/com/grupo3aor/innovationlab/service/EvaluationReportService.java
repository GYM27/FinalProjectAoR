...
public class EvaluationReportService {
    ...
    public EvaluationReportService(EvaluationReportRepository repository, EntityMapper mapper,
                                   SimulationRepository simulationRepository,
                                   AlertRepository alertRepository,
                                   ClinicalFormatter clinicalFormatter,
                                   RuleRepository ruleRepository,
                                   DataPersistenceComponent dataPersistenceComponent) {
    ...
    private byte[] generatePdfBytes(EvaluationReport report, Simulation simulation, Double startObservation, Double endObservation, Boolean isLive) {
        try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
            ...
            Font subtitleFont = FontFactory.getFont(FontFactory.HELVETICA_BOLD, SUBTITLE_FONT_SIZE);
            Font textFont = FontFactory.getFont(FontFactory.HELVETICA, TEXT_FONT_SIZE);
            Font boldFont = FontFactory.getFont(FontFactory.HELVETICA_BOLD, TEXT_FONT_SIZE);
            Font disclaimerFont = FontFactory.getFont(FontFactory.HELVETICA_OBLIQUE, DISCLAIMER_FONT_SIZE, Color.DARK_GRAY);
            ...
            if (simulation.getStartedAt() != null) {
                if (startObservation != null) {
                    startCutOff = simulation.getStartedAt().plusNanos((long)(startObservation * NANOS_PER_SECOND));
                }
                if (endObservation != null) {
                    endCutOff = simulation.getStartedAt().plusNanos((long)(endObservation * NANOS_PER_SECOND)).plusSeconds(1);
                }
            }
            ...
            // Merge alerts that are stuck in RAM due to Degraded Mode
            List<Alert> queuedAlerts = dataPersistenceComponent.getQueuedAlertsForSimulation(simulation.getId());
            if (queuedAlerts != null && !queuedAlerts.isEmpty()) {
                if (alerts == null) alerts = new ArrayList<>();
                // If there's an endCutOff, we must filter the RAM alerts too
                if (endCutOff != null) {
                    final LocalDateTime cutOff = endCutOff;
                    queuedAlerts = queuedAlerts.stream().filter(a -> a.getTimestamp() == null || !a.getTimestamp().isAfter(cutOff)).collect(Collectors.toList());
                }
                alerts.addAll(queuedAlerts);
            }
            ...
            if (alerts == null || alerts.isEmpty()) {
                ...
                List<ReportEvent> events = buildReportEvents(alerts, startCutOff, endCutOff);

                for (ReportEvent ev : events) {
                    String instante = "N/A";
                    
                    boolean effectiveIsLive = Boolean.TRUE.equals(isLive);
                    if (simulation.getScenario() != null && "Livestream".equalsIgnoreCase(simulation.getScenario().getName())) {
                        effectiveIsLive = true;
                    }

                    if (effectiveIsLive) {
                        if (ev.time != null) {
                            LocalDateTime displayTime = ev.time;
                            // Fix for UTC to LocalTime timezone difference from IoT streams
                            if (displayTime.getHour() < startBaseTime.getHour() && displayTime.plusHours(1).isAfter(startBaseTime.minusMinutes(5))) {
                                displayTime = displayTime.plusHours(1);
                            }
                            instante = displayTime.format(timeFormatter);
                        }
                    } else {
                        if (ev.time != null) {
                            long diffSecs = Duration.between(startBaseTime, ev.time).getSeconds();
                            // Quick timezone heuristic fix for diff calculation
                            if (diffSecs < 0 && ev.time.plusHours(1).isAfter(startBaseTime)) {
                                diffSecs = Duration.between(startBaseTime, ev.time.plusHours(1)).getSeconds();
                            }
    ...
    private List<ReportEvent> buildReportEvents(List<Alert> alerts, LocalDateTime startCutOff, LocalDateTime endCutOff) {
    ...
    private static class ReportEvent implements Comparable<ReportEvent> {
        LocalDateTime time;
        String systemName;
        String ruleName;
        String type;
