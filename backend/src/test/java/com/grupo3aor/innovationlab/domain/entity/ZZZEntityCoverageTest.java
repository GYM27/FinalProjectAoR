...
class ZZZEntityCoverageTest {
    ...
    @Test
    @DisplayName("Rule isApplicableTo logic")
    void testRuleIsApplicableTo() {
        Rule rule = new Rule();
        assertThat(rule.isApplicableTo("HR")).isFalse();

        rule.setExpressionDsl("invalid-dsl");
        assertThat(rule.isApplicableTo("HR")).isFalse();

        rule.setExpressionDsl("{\"metric\":\"HEART_RATE\"}");
        assertThat(rule.isApplicableTo("HeartRate")).isTrue();
        assertThat(rule.isApplicableTo("HR")).isTrue();
        assertThat(rule.isApplicableTo("Cardiovascular")).isTrue();
        assertThat(rule.isApplicableTo("SpO2")).isFalse();

        rule.setExpressionDsl("{\"metric\":\"SPO2\"}");
        assertThat(rule.isApplicableTo("OxygenSaturation")).isTrue();
        assertThat(rule.isApplicableTo("SpO2")).isTrue();
        assertThat(rule.isApplicableTo("Respiratory")).isTrue();
        assertThat(rule.isApplicableTo("HR")).isFalse();

        rule.setExpressionDsl("{\"metric\":\"BP\"}");
        assertThat(rule.isApplicableTo("ArterialPressure_Systolic")).isTrue();
        assertThat(rule.isApplicableTo("SystolicArterialPressure")).isTrue();
        assertThat(rule.isApplicableTo("SBP")).isTrue();
        assertThat(rule.isApplicableTo("HR")).isFalse();

        rule.setExpressionDsl("{\"metric\":\"RR\"}");
        assertThat(rule.isApplicableTo("RespirationRate")).isTrue();
        assertThat(rule.isApplicableTo("RR")).isTrue();
        assertThat(rule.isApplicableTo("HR")).isFalse();

        rule.setExpressionDsl("{\"metric\":\"TEMP\"}");
        assertThat(rule.isApplicableTo("CoreTemperature")).isTrue();
        assertThat(rule.isApplicableTo("TEMP")).isTrue();
        assertThat(rule.isApplicableTo("HR")).isFalse();
    }

    @Test
    @DisplayName("Rule isResolvedBy logic")
    void testRuleIsResolvedBy() {
        Rule rule = new Rule();
        assertThat(rule.isResolvedBy("HR", 100.0)).isFalse();

        rule.setExpressionDsl("invalid-dsl");
        assertThat(rule.isResolvedBy("HR", 100.0)).isFalse();

        // Testing ">" resolution (drops below threshold)
        rule.setExpressionDsl("{\"metric\":\"HEART_RATE\",\"operator\":\">\",\"resolutionThreshold\":90.0}");
        assertThat(rule.isResolvedBy("HR", 80.0)).isTrue();
        assertThat(rule.isResolvedBy("HR", 95.0)).isFalse();

        // Testing "<" resolution (goes above threshold)
        rule = new Rule();
        rule.setExpressionDsl("{\"metric\":\"SPO2\",\"operator\":\"<\",\"resolutionThreshold\":95.0}");
        assertThat(rule.isResolvedBy("SpO2", 96.0)).isTrue();
        assertThat(rule.isResolvedBy("SpO2", 90.0)).isFalse();

        // Testing "==" resolution
        rule = new Rule();
        rule.setExpressionDsl("{\"metric\":\"BP\",\"operator\":\"==\",\"resolutionThreshold\":120.0}");
        assertThat(rule.isResolvedBy("SBP", 130.0)).isTrue();
        assertThat(rule.isResolvedBy("SBP", 120.0)).isFalse();

        // Invalid operator
        rule = new Rule();
        rule.setExpressionDsl("{\"metric\":\"HR\",\"operator\":\"!=\",\"resolutionThreshold\":100.0}");
        assertThat(rule.isResolvedBy("HR", 150.0)).isFalse();
        
        // No resolution defined -> behaves as true
        rule = new Rule();
        rule.setExpressionDsl("{\"metric\":\"HR\",\"operator\":\">\"}");
        assertThat(rule.isResolvedBy("HR", 150.0)).isTrue();
    }

    @Test
    @DisplayName("Rule getResolutionPersistence logic")
    void testRuleGetResolutionPersistence() {
        Rule rule = new Rule();
        assertThat(rule.getResolutionPersistence()).isEqualTo(0);
        
        rule.setExpressionDsl("invalid-dsl");
        assertThat(rule.getResolutionPersistence()).isEqualTo(0);
        
        rule.setExpressionDsl("{\"metric\":\"HEART_RATE\",\"resolutionPersistence\":15}");
        assertThat(rule.getResolutionPersistence()).isEqualTo(15);
