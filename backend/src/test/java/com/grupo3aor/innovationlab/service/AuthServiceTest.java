...
import com.grupo3aor.innovationlab.exception.AccountAlreadyActivatedException;
...
class AuthServiceTest {
    ...
    void activateAccount_AlreadyActivatedOrInvalidToken() {
        ...
        AccountAlreadyActivatedException exception = assertThrows(AccountAlreadyActivatedException.class, () ->
    ...
    void authenticateUser_SoftDeletedUser() {
        // Due to @SQLRestriction("active = true") on the User entity, JPA will 
        // return Optional.empty() for logically deleted users.
        when(userRepository.findByEmail(anyString())).thenReturn(Optional.empty());
    ...
    void forgotPassword_SoftDeletedUser_ShouldNotThrow() {
        // If the account was Soft Deleted, the repository returns empty. 
        // The system pretends it sent to avoid revealing to hackers that the account exists but is inactive.
