...
export default function ResetPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!token) {
      setStatus({ type: "error", message: "Token de recuperação inválido ou inexistente." });
      return;
    }

    if (newPassword !== confirmPassword) {
      setStatus({ type: "error", message: "As palavras-passe não coincidem." });
      return;
    }
    ...
    try {
      const response = await fetchClient.post('/auth/reset-password', { token, newPassword });
      setStatus({ type: "success", message: response.message || "Palavra-passe redefinida com sucesso." });
      setSuccess(true);
            ...
            {!success && (
              <form onSubmit={handleSubmit} className="space-y-6 w-full">
                ...
                <button
                  ...
                  type="submit"
                >
                  {loading ? 'A PROCESSAR...' : 'REDEFINIR ACESSO'}
                </button>
              </form>
            )}
