...
(Referenciar os scripts de criação localizados em \texttt{src/main/resources/schema.sql} e a representação das tabelas nas classes da pasta \texttt{domain.entity} do servidor Java).

\section{System Screenshots \& User Manual}
(Colocar nesta secção as capturas de ecrã do Dashboard Principal, o destaque do corpo humano em alerta, e o exemplo do relatório final PDF que a aplicação emite quando a simulação termina).

\section{Automated Quality Reports}
(Incluir aqui a referência aos relatórios e gráficos gerados pela ferramenta de testes automatizada durante a execução do projeto).
